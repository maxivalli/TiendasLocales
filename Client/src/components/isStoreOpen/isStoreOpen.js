export default function isStoreOpen(dias, horarios) {
  ...
  currentDayIndex <= 0 ? currentDayIndex = 6 : currentDayIndex
  const currentDay = diasSemana[currentDayIndex];
  ...
  if (matchDays) {
    ...
    if (primerDiaIndex !== -1 && ultimoDiaIndex !== -1) {
      ...
      if (
        currentDayIndex >= primerDiaIndex &&
        currentDayIndex <= ultimoDiaIndex
      ) {
        ...
        if (horarios.horario_de_apertura2 && horarios.horario_de_cierre2) {
          const openingTime2 = parseInt(
            horarios.horario_de_apertura2.replace(":", ""),
            10
          );
          const closingTime2 = parseInt(
            horarios.horario_de_cierre2.replace(":", ""),
            10
          );
